...
class Checkout extends Component {
    ...
    return ({
      headerLeft: (
        <TouchableOpacity style={styles.headerLeft} onPress={() => navigation.goBack()}>
          <Ionicons style={styles.backIcon} name="ios-arrow-back" size={30} color="#fff"/>
        </TouchableOpacity>
      ),
      headerTitle: (
        <CustomText style={[commonStyles.titleCenter, { color: '#fff' }]}>Checkout</CustomText>
      ),
      headerRight: (
  ...
  onNextConfirm (orderId, createdAt) {
   
    this.setState({ confirmOrderId: orderId, confirmCreatedAt: createdAt }, () => {
  ...
  render() {
    ...
    const { currentIndex, confirmOrderId, confirmCreatedAt } = this.state;
    
    const screenToGoBack = _.get(navigation, 'state.key');
    let steps = [
      { label: 'Address', icon: Images.ICON_PIN },
      { label: 'Payment', icon: Images.ICON_MONEY },
      { label: 'Confirm', icon: Images.ICON_CART },
      { label: 'Successfully', icon: Images.ICON_FLAG },
    ];
      ...
      <View style={{ flex: 1 }}>
        ...
        <ScrollableTabView
          ...
          <Confirmation
            ...
            onNext={(orderId, createdAt) => this.onNextConfirm(orderId, createdAt)}
            carts={this.props.carts}/>
          <CheckoutFinish
            orderId={confirmOrderId}
            createdAt={confirmCreatedAt}
            navigation={this.props.navigation} />
        </ScrollableTabView>
        <DropdownAlert ref={ref => this.dropDownAlert = ref} closeInterval={3000} messageNumOfLines={7}
                       errorColor="#fe6c6c" translucent updateStatusBar={false}/>
      </View>
    );
  }
}
